        ...
        with st.spinner("350銘柄を調査中...（約2分かかります）"):
            ...
            for ticker in tickers_350:
                try:
                    ...
                    name = name.split(" Co").split(" CO").split(" Ltd").split(" LTD").strip()
            ...
            # データフォーマットの最適化（1円まで確実にテキスト化してAIに渡します）
            ...
            # 【命令のアップデート】スマホなどの表示サイズに関わらず、1円単位で正確に数値を出力する指示を強化しました
            prompt = f"""
            あなたはプロの極めて優秀な投資コンサルタントです。
            プログラムによる一次スクリーニングを通過した以下の3つのリストについて、Google検索で最新ニュースや決算をリアルタイム分析した上で、日本語で詳細に解説・提案してください。

            【絶対厳守ルール：株価の表記について】
            あなたが最終的に厳選した提案銘柄について、解説文の中で言及する際は、提供されたデータ末尾の「[現在株価 〇〇円]」の数値を【絶対に1円単位までそのまま、省略せずに正確に】記載してください。
            スマホ画面のサイズを気にして「大体いくらくらい」「〇〇円前後」といったマイルド・曖昧な要約表現に丸めることは【絶対に禁止】します。
            必ず「銘柄名 (コード) [現在の正確な株価: 〇〇.〇円]」の形で、厳格に事実のみを出力してください。

            1.【高配当・割安枠】から、今最もおすすめする銘柄を【1〜2つ】厳選とその詳細な理由（1円単位の正確な現在価格を明記すること）。
...
with tab2:
    ...
    if st.button("🚀 ネットの海から注目株の自動発掘を開始", type="primary", key="btn_trend"):
        ...
        with st.spinner("🔍 Geminiが東証のリアルタイム出来高や最新ニュースをネットパトロール中...（約1分）"):
            ...
            ※紹介する銘柄について、株価データが判明した場合はその現在株価を「大体」ではなく正確な数値で記載してください。
